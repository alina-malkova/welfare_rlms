...
%===============================================================================
\section{R4: Permutation Test and Wald Inference}
%===============================================================================

\subsection{Motivation}
Standard inference assumes asymptotic normality. We examine both permutation tests and standard Wald inference for testing the asymmetry hypothesis $H_0: \delta^+ = \delta^-$.

\subsection{Methodology}
The Wald test directly compares the interaction coefficients from the asymmetric smoothing regression:
\begin{equation}
\Delta\ln C = \alpha + \beta^+ \Delta\ln Y^+ + \beta^- \Delta\ln Y^- + \delta^+ (\Delta\ln Y^+ \times \text{Informal}) + \delta^- (\Delta\ln Y^- \times \text{Informal}) + X'\theta + \mu_i + \varepsilon
\end{equation}
...
\midrule
$\beta^+$ (formal, positive shocks) & 0.129 & 0.007 & 19.46 & $<0.001$ \\
$\beta^-$ (formal, negative shocks) & 0.165 & 0.009 & 19.21 & $<0.001$ \\
$\delta^+$ (informal $\times$ positive) & 0.003 & 0.014 & 0.20 & 0.840 \\
$\delta^-$ (informal $\times$ negative) & 0.027 & 0.019 & 1.46 & 0.144 \\
\midrule
\multicolumn{5}{l}{\textit{Tests:}} \\
$H_0: \delta^+ = \delta^-$ (Wald) & \multicolumn{4}{c}{$F = 0.91$, $p = 0.341$} \\
Observed $(\delta^- - \delta^+)$ & \multicolumn{4}{c}{0.024} \\
\midrule
$N$ & \multicolumn{4}{c}{138,747} \\
Clusters (individuals) & \multicolumn{4}{c}{19,285} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Key Finding}
The Wald test \textbf{cannot reject} $H_0: \delta^+ = \delta^-$ at conventional significance levels ($p = 0.341$). While informal workers show a point estimate of larger response to negative shocks ($\delta^- = 0.027$) than positive shocks ($\delta^+ = 0.003$), this difference is not statistically significant.

\textbf{Interpretation:} The lack of significance may reflect:
\begin{enumerate}
    \item Limited statistical power to detect the interaction effect
    \item True asymmetry that is smaller than hypothesized
    \item Substantial heterogeneity in the informal sector
\end{enumerate}

Note that the \textit{baseline} formal worker coefficients show clear asymmetry ($\beta^- = 0.165 > \beta^+ = 0.129$), consistent with universal loss aversion. The question is whether informal workers exhibit \textit{additional} asymmetry beyond this baseline.
...
\subsection{Motivation}
The event study design examines consumption dynamics around large negative income shocks (defined as $>$20\% income decline), comparing formal vs informal workers.

\subsection{Methodology}
We estimate:
\begin{equation}
\Delta\ln C_{it} = \sum_{k=-5}^{5} \beta_k \cdot \mathbf{1}[\text{event time} = k] + \mu_i + \tau_t + \varepsilon_{it}
\end{equation}
where event time $k = 0$ is the year of first large negative shock. Reference period is $k = -1$.

\subsection{Results}

\begin{table}[htbp]
\centering
\caption{Event Study Coefficients: Consumption Response to Income Shocks}
\label{tab:r5_results}
\begin{tabular}{lccccc}
\toprule
Event Time & Full Sample & Formal & Informal & Interaction & $p$-value \\
\midrule
$t = -5$ & $-0.011$ & 0.009 & $-0.028$ & & \\
$t = -4$ & $-0.016$** & $-0.019$* & $-0.006$ & & \\
$t = -3$ & $-0.009$ & $-0.011$ & 0.006 & & \\
$t = -2$ & $-0.005$ & $-0.009$ & 0.003 & & \\
$t = -1$ & 0 & 0 & 0 & & (ref) \\
\midrule
$t = 0$ & $-0.088$*** & $-0.087$*** & $-0.103$*** & $-0.031$*** & 0.004 \\
$t = +1$ & 0.000 & $-0.001$ & $-0.003$ & $-0.014$ & 0.196 \\
$t = +2$ & $-0.016$*** & $-0.017$*** & $-0.022$* & $-0.010$ & 0.358 \\
$t = +3$ & 0.002 & 0.004 & 0.003 & $-0.013$ & 0.265 \\
\midrule
$N$ & 90,325 & 65,714 & 21,033 & & \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
\item Notes: Dependent variable is $\Delta\ln(\text{Consumption})$. Individual and year FE. Clustered SE by individual. *** $p<0.01$, ** $p<0.05$, * $p<0.10$. Interaction column shows informal worker penalty relative to formal workers.
\end{tablenotes}
\end{table}
...
\begin{enumerate}
    \item \textbf{Pre-trends are parallel:} Coefficients for $t = -5$ to $t = -2$ are all close to zero and not jointly significant, supporting the identification assumption.

    \item \textbf{Large consumption drop at shock:} At $t = 0$, consumption falls by 8.8\% on average. Formal workers experience an 8.7\% decline; informal workers experience a 10.3\% decline.

    \item \textbf{Significant informal penalty at impact:} The interaction term shows informal workers experience an \textbf{additional 3.1 percentage point consumption decline} at the time of shock ($p = 0.004$), relative to formal workers facing the same income shock.

    \item \textbf{Recovery is similar:} Post-shock coefficients ($t = +1$ to $t = +3$) do not show significant informal-formal differences, suggesting the penalty is concentrated at impact rather than persistent.
\end{enumerate}

\textbf{Interpretation:} The event study provides clean causal evidence that informal workers face worse consumption outcomes when hit by negative income shocks. The effect is concentrated at the time of shock, consistent with differential ability to access credit or savings buffers to smooth the initial impact.
...
R4 & Wald test & $p = 0.341$ & Asymmetry not significant \\
R5 & Event study & $-3.1$pp penalty & Impact effect confirmed \\
